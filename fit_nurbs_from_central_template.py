...
# -*- coding: utf-8 -*-
r"""
Fit cubic tensor-product B-spline (NURBS) surfaces to the central template mesh
for any supported ROI using quad-grid control meshes (no LSCM), and verify the
fitting accuracy in 3D via Chamfer distance. Output files are named with the
anatomical ROI prefix.
...
5. 输出（以 femoral_cartilage 为例，会根据 ROI 自动更名）：
   - femoral_cartilage_template_surf_patch0.npz / patch1.npz (/ patch2.npz)
   - femoral_cartilage_template_nurbs_mesh_multi.ply   （所有 patch 的 NURBS 网格合在一起）
   - femoral_cartilage_template_chamfer_stats.txt      （Chamfer 统计）

Example:

python fit_nurbs_from_central_template.py ^
  --roi femoral_cartilage ^
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Fit multi-patch NURBS surfaces from central templates (configurable ROI, quad control meshes, no LSCM)."
    )
    p.add_argument(
        "--roi",
        type=str,
        ...
        help=(
            "ROI id or anatomical name for labeling outputs. Supported: "
...
def save_nurbs_template(surf, out_dir: Path, patch_id: int, template_prefix: str):
    """
    每个 patch 单独保存一个 npz:
      <roi>_template_surf_patch{patch_id}.npz
    """
    ...
    np.savez(
        out_dir / f"{template_prefix}_surf_patch{patch_id}.npz",
        ctrlpts=ctrlpts_3d,
        knots_u=knots_u,
        knots_v=knots_v,
        degree_u=degree_u,
    ...
    print(
        ...
        f"{out_dir / f'{template_prefix}_surf_patch{patch_id}.npz'}"
    )
...
def main():
    ...
    template_prefix = f"{roi_name}_template"
    ...
    print(
        f"Splitting ROI={roi_id} ({roi_name}) mesh into {args.n_patches} longitudinal patches "
        "(fixed axis matching training pipeline)..."
    )
    ...
    for pid, patch_mesh in enumerate(patch_meshes):
        ...
        save_nurbs_template(surf, out_dir, patch_id=pid, template_prefix=template_prefix)
    ...
    stats_path = out_dir / f"{template_prefix}_chamfer_stats.txt"
    ...
    nurbs_mesh_path = out_dir / f"{template_prefix}_nurbs_mesh_multi.ply"
