...
Workflow:
  ...
  -> mean surface (inner_mesh_mc) -> PCA-based (u,v) parameterization
  -> least-squares fit cubic tensor-product B-spline surface
  -> evaluate NURBS on regular (u,v) grid -> inner template mesh
  -> optional outer surface by normal offset (~2 mm)
  -> save meshes (.ply) + NURBS params (.npz) + manifest (.json)

Run (Windows CMD, one line):
  python build_femoral_cartilage_template.py ^
      --data_root C:\\path\\to\\OAI-ZIB-CM ^
      --out_dir C:\\path\\to\\Carti_Seg ^
...
def build_template_for_roi(
    ...
    inner_mesh_mc = smooth_mesh(mean_mesh_mc, method=smooth_method,
                                iterations=max(1, smooth_iters // 2),
                                lambda_=taubin_lambda, mu=taubin_mu)

    # -------------------------------------------------
    # NURBS fitting on mean mesh (inner surface)
    # -------------------------------------------------
    print("Fitting cubic tensor-product B-spline surface (NURBS template)...")
    inner_verts = np.asarray(inner_mesh_mc.vertices)   # (V,3)

    # 1) Parameterize vertices -> (u,v) in [0,1]^2
    uv = pca_parameterize_vertices(inner_verts)         # (V,2)
    ...
    # 3) Evaluate surface on regular (u,v) grid to build template inner mesh
    inner_eval_verts, inner_eval_faces = evaluate_bspline_surface_grid(
        ctrl_pts,
        knots_u,
        knots_v,
        degree_u=nurbs_degree_u,
    ...
    inner_mesh = o3d.geometry.TriangleMesh(
        vertices=o3d.utility.Vector3dVector(inner_eval_verts),
        triangles=o3d.utility.Vector3iVector(inner_eval_faces.astype(np.int32)),
    )
    inner_mesh.remove_degenerate_triangles()
    inner_mesh.compute_vertex_normals()

    # Outer (offset) mesh in physical space
    outer_mesh = None
    if offset_outer_mm and offset_outer_mm > 0:
        print(f"Creating outer surface by normal offset of {offset_outer_mm:.2f} mm...")
    ...
    print("\nDone.")
    print(f"Saved inner NURBS template mesh: {inner_path}")
    if outer_path is not None:
        print(f"Saved outer surface mesh: {outer_path}")
...
def parse_args():
    p = argparse.ArgumentParser(description="Build femoral cartilage (ROI=2) NURBS template from OAI-ZIB-CM")
